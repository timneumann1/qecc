...
def main() -> None:
    if len(sys.argv)>2:    
        qasm = sys.argv[1]
        distance = int(sys.argv[2]) # used to determine the max. number of errors, which is in turn used to determine the fault sets, etc.
        method = sys.argv[3]
    ...
    try:
        ...
        non_ft_sp = FaultyStatePrepCircuit(non_ft_sp, (distance-1)//2, (distance-1)//2)
        if method == "optimal":
            ft_sp = gate_optimal_verification_circuit(non_ft_sp, verify_x_first=True) # first perform Z-type measurements (to verify X errors)
